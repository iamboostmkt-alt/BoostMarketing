          ...
          <div>
            <div className="flex items-center gap-2 flex-wrap mb-3">
              ...
              <Select value={teamStatusFilter} onValueChange={setTeamStatusFilter}>
                <SelectTrigger className="h-7 w-32 bg-white/[0.04] border-[var(--wl-border)] text-white text-xs">
                  <SelectValue />
                </SelectTrigger>
              ...
              {teamUsers.length > 0 && (
                <Select value={teamUserFilter} onValueChange={setTeamUserFilter}>
                  <SelectTrigger className="h-7 w-36 bg-white/[0.04] border-[var(--wl-border)] text-white text-xs">
                    <SelectValue />
                  </SelectTrigger>
                  <SelectContent className="bg-[var(--wl-surface)] border-[var(--wl-border)] text-white">
            ...
            <div className="flex gap-1">
              {tabs.map(tab => (
                <button key={tab.id} onClick={() => setTaskTab(tab.id)}
                  className={`flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-medium transition-all ${taskTab === tab.id ? 'text-white' : 'text-[var(--wl-text-muted)] hover:text-[var(--wl-text-secondary)]'}`}
                          ...
                          <div className="flex items-center gap-1">
                            ...
                            {ri > 0 && (
                              <button
                                ...
                                className="flex items-center gap-1 text-[10px] text-white/30 hover:text-violet-400 transition-colors px-1.5 py-0.5 rounded border border-[var(--wl-border)] hover:border-violet-500/30"
                          ...
                          <div className="flex items-center gap-1">
                            {!isFirst && (
                              <button onClick={() => moveSection(section.id, 'up')}
                                className="text-[10px] text-white/25 hover:text-violet-400 transition-colors w-5 h-5 flex items-center justify-center rounded border border-[var(--wl-border)]">
                                ←
                              </button>
                            )}
                            {!isLast && (
