...
function Avatar({ name, color, image, size = 24 }: { name: string; color: string; image?: string | null; size?: number }) {
  ...
  return (
    <div className="rounded-full flex items-center justify-center shrink-0 text-[var(--wl-text-primary)] font-semibold ring-1 ring-white/10"
    ...
    <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center bg-black/60 backdrop-blur-sm p-0 sm:p-4">
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: 20 }}
        ...
        <div className="flex items-center justify-between px-5 py-4 border-b border-[var(--wl-border)]">
          <h3 className="text-[15px] font-semibold text-[var(--wl-text-primary)]">{milestone ? 'Editar fase' : 'Nueva fase'}</h3>
          <button onClick={onClose} className="text-[var(--wl-text-placeholder)] hover:text-[var(--wl-text-primary)]"><X className="w-4 h-4" /></button>
        </div>
        <div className="p-5 space-y-4">
          <div>
            ...
            <input value={title} onChange={e => setTitle(e.target.value)}
              ...
              className="w-full rounded-xl bg-[var(--wl-hover)] border border-[var(--wl-border)] px-3 py-2.5 text-sm text-[var(--wl-text-primary)] placeholder:text-[var(--wl-text-placeholder)] outline-none focus:border-violet-500/50" />
          </div>
          <div>
            ...
            <textarea value={description} onChange={e => setDescription(e.target.value)} rows={2}
              ...
              className="w-full rounded-xl bg-[var(--wl-hover)] border border-[var(--wl-border)] px-3 py-2.5 text-sm text-[var(--wl-text-primary)] placeholder:text-[var(--wl-text-placeholder)] outline-none focus:border-violet-500/50 resize-none" />
          </div>
          <div className="grid grid-cols-2 gap-3">
            <div>
              ...
              <input type="date" value={date} onChange={e => setDate(e.target.value)}
                className="w-full rounded-xl bg-[var(--wl-hover)] border border-[var(--wl-border)] px-3 py-2.5 text-sm text-[var(--wl-text-primary)] outline-none focus:border-violet-500/50 [color-scheme:dark]" />
            </div>
            <div>
              <label className="text-[11px] text-[var(--wl-text-muted)] uppercase tracking-wider mb-1.5 block">Estado</label>
              <select value={status} onChange={e => setStatus(e.target.value)}
                className="w-full rounded-xl bg-[var(--wl-hover)] border border-[var(--wl-border)] px-3 py-2.5 text-sm text-[var(--wl-text-primary)] outline-none focus:border-violet-500/50">
          ...
          <div className="flex gap-3 pt-1">
            <button onClick={onClose} className="flex-1 rounded-xl border border-[var(--wl-border)] py-2.5 text-[13px] text-[var(--wl-text-muted)] hover:text-[var(--wl-text-primary)] transition-colors">
              Cancelar
            </button>
...
function MilestoneCard({ milestone, color, isManager, onEdit, onDelete, onStatusChange }: {
  ...
  return (
    <div className="rounded-2xl border border-[var(--wl-border)] bg-[var(--wl-hover)] overflow-hidden transition-all">
      {/* Header */}
      <div className="flex items-center gap-3 p-4">
        ...
        <div className="flex-1 min-w-0 cursor-pointer" onClick={() => setExpanded(v => !v)}>
          <div className="flex items-center gap-2">
            <p className={`text-[13px] font-medium leading-snug ${milestone.status === 'completed' ? 'text-[var(--wl-text-muted)] line-through' : 'text-[var(--wl-text-primary)]'}`}>
              {milestone.title}
            </p>
          ...
          <div className="flex items-center gap-3 mt-1">
            {milestone.date && (
              <span className="text-[11px] text-[var(--wl-text-placeholder)] flex items-center gap-1">
        ...
        {isManager && (
          <div className="flex items-center gap-1 shrink-0">
            <button onClick={onEdit} className="w-7 h-7 rounded-lg flex items-center justify-center text-[var(--wl-text-placeholder)] hover:text-[var(--wl-text-secondary)] hover:bg-[var(--wl-hover)] transition-colors">
              <Pencil className="w-3 h-3" />
            </button>
            <button onClick={onDelete} className="w-7 h-7 rounded-lg flex items-center justify-center text-[var(--wl-text-placeholder)] hover:text-red-400 hover:bg-red-500/10 transition-colors">
              <Trash2 className="w-3 h-3" />
            </button>
          </div>
        )}

        <button onClick={() => setExpanded(v => !v)} className="text-[var(--wl-text-placeholder)] shrink-0">
          {expanded ? <ChevronDown className="w-4 h-4" /> : <ChevronRight className="w-4 h-4" />}
        </button>
      </div>

      {/* Progress bar */}
      <div className="h-0.5 bg-[var(--wl-hover)]">
          ...
          <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: 'auto', opacity: 1 }}
            ...
            <div className="px-4 pb-3 pt-2 space-y-1.5">
              {milestone.tasks.length === 0 ? (
                <p className="text-[12px] text-[var(--wl-text-placeholder)] py-2 text-center">Sin tareas vinculadas a esta fase</p>
                ...
                return (
                  <div key={task.id} className="flex items-center gap-2.5 py-1.5 px-2 rounded-lg hover:bg-[var(--wl-hover)] transition-colors">
                    ...
                    <span className={`text-[12px] flex-1 truncate ${task.status === 'completed' || task.status === 'approved' ? 'text-[var(--wl-text-muted)] line-through' : 'text-[var(--wl-text-secondary)]'}`}>
                      {task.title}
                    </span>
    ...
    <div className="flex flex-col gap-0 min-h-full">
      ...
      <div className="px-4 sm:px-6 pt-5 pb-4 border-b border-[var(--wl-border-subtle)]"
        ...
        <button onClick={() => router.push('/dashboard/projects')}
          className="flex items-center gap-1.5 text-[12px] text-[var(--wl-text-placeholder)] hover:text-[var(--wl-text-secondary)] transition-colors mb-4">
        ...
        <div className="flex items-start justify-between gap-4">
          <div className="flex items-start gap-3 min-w-0">
            ...
            <div className="min-w-0">
              <h1 className="text-[20px] sm:text-[24px] font-bold text-[var(--wl-text-primary)] leading-tight truncate">
                {project.name}
              </h1>
              {project.client && (
              ...
              {project.description && (
                <p className="text-[13px] text-[var(--wl-text-muted)] mt-1.5 line-clamp-2">{project.description}</p>
              )}
            </div>
          </div>
          ...
          <div className="flex items-center gap-2 shrink-0">
            {isManager && (
              <select value={project.status} onChange={e => changeProjectStatus(e.target.value)}
                className="rounded-xl bg-[var(--wl-hover)] border border-[var(--wl-border)] px-2.5 py-1.5 text-[12px] text-[var(--wl-text-secondary)] outline-none focus:border-violet-500/40 [color-scheme:dark]">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mt-5">
          {[
            ...
            <div key={s.label} className="rounded-xl border border-[var(--wl-border-subtle)] bg-[var(--wl-hover)] px-3 py-3 flex items-center gap-2.5">
              <s.icon className="w-4 h-4 shrink-0" style={{ color: s.color }} />
              <div className="min-w-0">
                <p className="text-[10px] text-[var(--wl-text-placeholder)] uppercase tracking-wider">{s.label}</p>
                <p className="text-[15px] font-semibold text-[var(--wl-text-primary)]">{s.value}</p>
              </div>
            </div>
          ))}
        </div>
        ...
        {tabs.map(t => (
          <button key={t.id} onClick={() => setActiveTab(t.id)}
            className={`flex items-center gap-2 px-1 py-3 mr-6 text-[13px] font-medium border-b-2 -mb-px transition-all ${
              activeTab === t.id
                ? 'border-violet-500 text-[var(--wl-text-primary)]'
                : 'border-transparent text-[var(--wl-text-muted)] hover:text-[var(--wl-text-secondary)]'
            }`}>
            {t.label}
            <span className={`text-[10px] px-1.5 py-0.5 rounded-full ${
              activeTab === t.id ? 'bg-violet-500/20 text-violet-300' : 'bg-[var(--wl-hover)] text-[var(--wl-text-placeholder)]'
            }`}>
              {t.count}
            </span>
          </button>
      ...
      <div className="flex-1 px-4 sm:px-6 py-5">
        ...
        {activeTab === 'milestones' && (
          <div className="space-y-3">
            <div className="flex items-center justify-between mb-2">
              <p className="text-[12px] text-[var(--wl-text-placeholder)]">{milestones.length} fase{milestones.length !== 1 ? 's' : ''} del proyecto</p>
              {isManager && (
            ...
            {milestones.length === 0 ? (
              <div className="flex flex-col items-center justify-center py-16 gap-3 text-[var(--wl-text-placeholder)]">
        ...
        {activeTab === 'tasks' && (
          <div className="space-y-2">
            <div className="flex items-center justify-between mb-2">
              <p className="text-[12px] text-[var(--wl-text-placeholder)]">{totalTasks} tareas vinculadas al proyecto</p>
            </div>
            {(project.tasks ?? []).length === 0 ? (
              <div className="flex flex-col items-center justify-center py-16 gap-3 text-[var(--wl-text-placeholder)]">
                <CheckCircle2 className="w-10 h-10 opacity-40" />
                <p className="text-[14px]">Sin tareas en este proyecto</p>
                <p className="text-[12px] text-[var(--wl-text-placeholder)]">Asigna el projectId al crear tareas desde el kanban</p>
              </div>
            ) : (project.tasks ?? []).map(task => {
              const tc = TASK_STATUS_COLOR[task.status] || '#94a3b8';
              return (
                <div key={task.id}
                  className="flex items-center gap-3 p-3 rounded-xl border border-[var(--wl-border-subtle)] bg-[var(--wl-hover)] hover:bg-[var(--wl-hover)] transition-colors">
                  <div className="w-2.5 h-2.5 rounded-full shrink-0" style={{ background: tc }} />
                  <span className="flex-1 text-[13px] text-[var(--wl-text-secondary)] truncate">{task.title}</span>
                  ...
                  {task.dueDate && (
                    <span className="text-[11px] text-[var(--wl-text-placeholder)] shrink-0">
                      {new Date(task.dueDate).toLocaleDateString('es-MX', { day: 'numeric', month: 'short' })}
                    </span>
                  )}
                </div>
        ...
        {activeTab === 'team' && (
          <div className="space-y-2">
            <p className="text-[12px] text-[var(--wl-text-placeholder)] mb-3">{project.assignedUsers.length} miembros asignados</p>
            {project.assignedUsers.map(({ user }) => (
              <div key={user.id} className="flex items-center gap-3 p-3 rounded-xl border border-[var(--wl-border-subtle)] bg-[var(--wl-hover)]">
                ...
                <div>
                  ...
                  <p className="text-[11px] text-[var(--wl-text-placeholder)]">{user.role}</p>
                </div>
              </div>
            ))}
            {project.assignedUsers.length === 0 && (
              <div className="flex flex-col items-center justify-center py-16 text-[var(--wl-text-placeholder)] gap-2">
