    ...
    <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center bg-black/60 backdrop-blur-sm p-0 sm:p-4">
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: 20 }}
        ...
        <div className="p-5 space-y-4">
          <div>
            <label className="text-[11px] text-white/40 uppercase tracking-wider mb-1.5 block">Título *</label>
            <input value={title} onChange={e => setTitle(e.target.value)}
              placeholder="Ej. Entrega de diseños, Lanzamiento..."
              className="w-full rounded-xl bg-white/[0.04] border border-white/[0.07] px-3 py-2.5 text-sm text-white placeholder:text-white/20 outline-none focus:border-violet-500/50" />
          </div>
          <div>
            <label className="text-[11px] text-white/40 uppercase tracking-wider mb-1.5 block">Descripción</label>
            <textarea value={description} onChange={e => setDescription(e.target.value)} rows={2}
              placeholder="¿Qué incluye esta fase?"
              className="w-full rounded-xl bg-white/[0.04] border border-white/[0.07] px-3 py-2.5 text-sm text-white placeholder:text-white/20 outline-none focus:border-violet-500/50 resize-none" />
          </div>
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className="text-[11px] text-white/40 uppercase tracking-wider mb-1.5 block">Fecha límite</label>
              <input type="date" value={date} onChange={e => setDate(e.target.value)}
                className="w-full rounded-xl bg-white/[0.04] border border-white/[0.07] px-3 py-2.5 text-sm text-white outline-none focus:border-violet-500/50 [color-scheme:dark]" />
            </div>
            <div>
          ...
          <div>
            <div className="flex items-center justify-between mb-2">
              <label className="text-[11px] text-white/40 uppercase tracking-wider">Progreso</label>
          ...
          <div className="flex gap-3 pt-1">
            <button onClick={onClose} className="flex-1 rounded-xl border border-white/[0.07] py-2.5 text-[13px] text-white/50 hover:text-white transition-colors">
              Cancelar
            </button>
...
function MilestoneCard({ milestone, color, isManager, onEdit, onDelete, onStatusChange }: {
  ...
  return (
    <div className="rounded-2xl border border-white/[0.06] bg-white/[0.02] overflow-hidden transition-all">
      {/* Header */}
      <div className="flex items-center gap-3 p-4">
        ...
        <button onClick={() => {
          ...
          className="shrink-0 w-8 h-8 rounded-full flex items-center justify-center transition-colors hover:bg-white/[0.06]"
        ...
        <div className="flex-1 min-w-0 cursor-pointer" onClick={() => setExpanded(v => !v)}>
          <div className="flex items-center gap-2">
            <p className={`text-[13px] font-medium leading-snug ${milestone.status === 'completed' ? 'text-white/40 line-through' : 'text-white/85'}`}>
              {milestone.title}
            </p>
        ...
        {isManager && (
          <div className="flex items-center gap-1 shrink-0">
            <button onClick={onEdit} className="w-7 h-7 rounded-lg flex items-center justify-center text-white/30 hover:text-white/70 hover:bg-white/[0.06] transition-colors">
          ...
          <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: 'auto', opacity: 1 }}
            ...
            <div className="px-4 pb-3 pt-2 space-y-1.5">
              {milestone.tasks.length === 0 ? (
                ...
                return (
                  <div key={task.id} className="flex items-center gap-2.5 py-1.5 px-2 rounded-lg hover:bg-white/[0.03] transition-colors">
...
export default function ProjectDetailPage() {
  ...
  return (
    <div className="flex flex-col gap-0 min-h-full">
      ...
      {/* ── Header ── */}
      <div className="px-4 sm:px-6 pt-5 pb-4 border-b border-white/[0.05]"
        ...
        <div className="flex items-start justify-between gap-4">
          <div className="flex items-start gap-3 min-w-0">
            ...
            <div className="min-w-0">
              ...
              {project.client && (
                <p className="text-[13px] text-white/40 mt-0.5">{project.client.name}</p>
              )}
              {project.description && (
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mt-5">
          {[
            ...
            <div key={s.label} className="rounded-xl border border-white/[0.05] bg-white/[0.02] px-3 py-3 flex items-center gap-2.5">
      ...
      {/* ── Tabs ── */}
      <div className="flex border-b border-white/[0.05] px-4 sm:px-6">
        {tabs.map(t => (
        ...
        {activeTab === 'team' && (
          <div className="space-y-2">
            ...
            {project.assignedUsers.map(({ user }) => (
              <div key={user.id} className="flex items-center gap-3 p-3 rounded-xl border border-white/[0.05] bg-white/[0.02]">
