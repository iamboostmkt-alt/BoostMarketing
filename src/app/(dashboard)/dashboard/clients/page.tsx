...
const STATUS_META: Record<string, { label: string; color: string; dot: string }> = {
  ...
  inactive: { label: 'Inactivo',  color: 'text-[var(--wl-text-placeholder)]',    dot: 'bg-white/20'    },
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-2">
        <p className="text-[10px] uppercase tracking-widest text-[var(--wl-text-placeholder)]">Links esenciales</p>
        {isAdmin && !adding && (
      ...
      <div className="space-y-1.5">
        {links.map((l, i) => (
          <div key={i} className="flex items-center gap-2 glass-card rounded-lg px-3 py-2 group">
            ...
            <a href={l.url} target="_blank" rel="noopener noreferrer"
              className="flex-1 min-w-0 text-xs text-[var(--wl-text-secondary)] hover:text-[var(--wl-text-primary)] transition-colors truncate">
              {l.label}
            </a>
            <a href={l.url} target="_blank" rel="noopener noreferrer"
              className="opacity-0 group-hover:opacity-100 transition-opacity">
              <ExternalLink className="w-3 h-3 text-[var(--wl-text-placeholder)] hover:text-[var(--wl-text-secondary)]" />
            </a>
            {isAdmin && (
              <button onClick={() => removeLink(i)}
                className="opacity-0 group-hover:opacity-100 transition-opacity text-[var(--wl-text-placeholder)] hover:text-red-400">
                <X className="w-3 h-3" />
              </button>
            )}
          </div>
        ...
        {links.length === 0 && !adding && (
          <p className="text-[11px] text-[var(--wl-text-placeholder)] text-center py-2">Sin links agregados</p>
        )}
      </div>
      ...
      {adding && (
        <div className="mt-2 space-y-2 glass-card rounded-lg px-3 py-3">
            ...
            {LINK_PRESETS.map(p => (
              <button key={p.label} onClick={() => { setNewLabel(p.label); setNewIcon(p.icon); }}
                className={`flex items-center gap-1 px-2 py-1 rounded-md text-[11px] transition-colors ${
                  newLabel === p.label ? 'bg-violet-600/30 text-violet-300 border border-violet-500/40' : 'bg-[var(--wl-hover)] text-[var(--wl-text-muted)] hover:text-[var(--wl-text-secondary)]'
                }`}>
          ...
          <input value={newUrl} onChange={e => setNewUrl(e.target.value)}
            ...
            className="w-full rounded-lg bg-[var(--wl-hover)] border border-[var(--wl-border)] px-3 py-1.5 text-xs text-[var(--wl-text-primary)] placeholder:text-[var(--wl-text-placeholder)] focus:outline-none focus:border-violet-500/50" />
          <div className="flex gap-2">
            ...
            <button onClick={() => { setAdding(false); setNewLabel(''); setNewUrl(''); }}
              className="px-3 py-1.5 rounded-lg bg-[var(--wl-hover)] text-xs text-[var(--wl-text-muted)] hover:text-[var(--wl-text-secondary)] transition-colors">
              Cancelar
            </button>
          </div>
        </div>
      ...
      <div className="relative h-full w-full max-w-sm bg-[#0f0f13] border-l border-[var(--wl-border)] overflow-y-auto shadow-2xl"
        ...
        <div className="flex items-center justify-between px-5 py-4 border-b border-[var(--wl-border)]">
          ...
          <div className="flex items-center gap-1">
            <button onClick={() => onEdit(client)} className="p-1.5 rounded-lg text-[var(--wl-text-placeholder)] hover:text-[var(--wl-text-primary)] hover:bg-[var(--wl-hover)] transition-colors">
              <Pencil className="w-4 h-4" />
            </button>
            {isAdmin && (
              <button onClick={() => onDelete(client)} className="p-1.5 rounded-lg text-[var(--wl-text-placeholder)] hover:text-red-400 hover:bg-red-400/10 transition-colors">
                <Trash2 className="w-4 h-4" />
              </button>
            )}
            <button onClick={onClose} className="p-1.5 rounded-lg text-[var(--wl-text-placeholder)] hover:text-[var(--wl-text-primary)] hover:bg-[var(--wl-hover)] transition-colors">
              <X className="w-4 h-4" />
            </button>
          </div>
        </div>
        ...
        <div className="px-5 py-5 space-y-6">
          <div className="flex items-center gap-4">
            ...
            <div className="min-w-0">
              <p className="text-base font-medium text-[var(--wl-text-primary)] truncate">{client.name}</p>
          ...
          <div className="space-y-3">
            <p className="text-[10px] uppercase tracking-widest text-[var(--wl-text-placeholder)]">Informacion</p>
            {[
              ...
              <div key={row.label} className="flex items-start gap-3">
                <row.icon className="w-3.5 h-3.5 text-[var(--wl-text-placeholder)] mt-0.5 shrink-0" />
                <div className="min-w-0">
                  <p className="text-[10px] text-[var(--wl-text-placeholder)]">{row.label}</p>
          ...
          <div className="grid grid-cols-2 gap-3">
            <div className="glass-card rounded-lg px-3 py-2.5">
              <p className="text-[10px] text-[var(--wl-text-placeholder)] mb-1">Tareas activas</p>
              <p className="text-xl font-medium text-cyan-400">{taskCount === null ? '...' : taskCount}</p>
            </div>
            <div className="glass-card rounded-lg px-3 py-2.5">
              <p className="text-[10px] text-[var(--wl-text-placeholder)] mb-1">Equipo asignado</p>
          ...
          {(client as any).assignedManager && (
            <div>
              <p className="text-[10px] uppercase tracking-widest text-[var(--wl-text-placeholder)] mb-2">Project Manager</p>
              <div className="flex items-center gap-3 glass-card rounded-lg px-3 py-2.5">
                ...
                <div className="min-w-0">
                  ...
                  <p className="text-[11px] text-[var(--wl-text-placeholder)] truncate">{(client as any).assignedManager.email}</p>
                </div>
              </div>
            </div>
          )}
          ...
          {((client as any).assignedUsers ?? []).length > 0 && (
            <div>
              <p className="text-[10px] uppercase tracking-widest text-[var(--wl-text-placeholder)] mb-2">Equipo asignado</p>
                ...
                {((client as any).assignedUsers ?? []).map((u: any) => (
                  <div key={u.id} className="flex items-center gap-3 glass-card rounded-lg px-3 py-2">
                    ...
                    <div className="min-w-0">
                      ...
                      <p className="text-[10px] text-[var(--wl-text-placeholder)] truncate">{u.email}</p>
                    </div>
                  </div>
                ))}
              </div>
        ...
        {assignedUsers.length > 0 && (
          <div className="flex -space-x-1.5 mb-0.5">
            ...
            {assignedUsers.length > 4 && (
              <div className="flex h-6 w-6 items-center justify-center rounded-full border border-[#0a0a0f] bg-[var(--wl-border)] text-[8px] text-[var(--wl-text-muted)]">
                +{assignedUsers.length - 4}
              </div>
            )}
          </div>
      ...
      <div className="relative z-10 flex flex-1 flex-col px-3 pt-1.5 pb-3">
        <div className="mb-2">
          <div className="flex items-center gap-1.5 flex-wrap">
            <h3 className="text-[13px] font-bold text-[var(--wl-text-primary)] truncate">{client.name}</h3>
          ...
          {(client as any).company && <p className="text-[11px] text-[var(--wl-text-muted)] truncate">{(client as any).company}</p>}
          <p className="text-[10px] text-[var(--wl-text-placeholder)] truncate mt-0.5">{client.email}</p>
        </div>
        {/* Stats */}
        ...
        <div className="mt-auto flex items-center justify-between">
          ...
          ) : <span className="text-[9px] text-[var(--wl-text-placeholder)]">Sin PM</span>}
                  ...
                  >
                    <div className="py-1">
                      <button onClick={() => { setMenuOpen(false); onPortal(); }}
                        className="w-full px-3 py-2 text-left text-[12px] text-[var(--wl-text-secondary)] transition-colors hover:bg-[var(--wl-hover)] hover:text-[var(--wl-text-primary)]">
                        Ver portal
                      </button>
  ...
  return (
    <div className="space-y-4 sm:space-y-6 px-3 sm:px-6 py-4 sm:py-6">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          <p className="text-xs font-medium text-[var(--wl-text-placeholder)] uppercase tracking-widest mb-1">{label}</p>
          <h1 className="text-xl font-medium text-[var(--wl-text-primary)]">{isAdmin ? 'Gestion de clientes' : 'Mis cuentas'}</h1>
      ...
      <div className="flex flex-col sm:flex-row gap-3 items-start sm:items-center">
        ...
        <div className="flex gap-1 bg-[var(--wl-hover)] rounded-lg p-0.5 border border-[var(--wl-border)]">
          {TABS.map(tab => (
            <button key={tab.key} onClick={() => setActiveTab(tab.key)}
              className={"px-3 py-1.5 rounded-md text-xs font-medium transition-all " + (activeTab === tab.key ? 'bg-[var(--wl-border)] text-[var(--wl-text-primary)]' : 'text-[var(--wl-text-placeholder)] hover:text-[var(--wl-text-secondary)]')}>
              {tab.label}
      ...
      {loading ? (
        ...
        <div className="flex flex-col items-center justify-center py-20 text-center">
          <Building2 className="w-12 h-12 text-[var(--wl-text-placeholder)] mb-3" />
          ...
          {activeTab === 'all' && !search && isManager && (
            <button onClick={handleCreate} className="mt-4 flex items-center gap-2 px-4 py-2 rounded-lg border border-[var(--wl-border)] text-[var(--wl-text-muted)] hover:text-[var(--wl-text-primary)] hover:border-white/20 text-sm transition-colors">
      ...
      <AlertDialog open={!!deleteTarget} onOpenChange={o => !o && setDeleteTarget(null)}>
        <AlertDialogContent className="bg-[var(--wl-surface)] border-[var(--wl-border)] text-[var(--wl-text-primary)]">
          <AlertDialogHeader>
            <AlertDialogTitle className="text-[var(--wl-text-primary)]">Eliminar cliente?</AlertDialogTitle>
          ...
          <AlertDialogFooter>
            <AlertDialogCancel className="border-white/[0.1] text-[var(--wl-text-secondary)] hover:text-[var(--wl-text-primary)] hover:bg-[var(--wl-hover)]">Cancelar</AlertDialogCancel>
