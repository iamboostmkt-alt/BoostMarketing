...
const VALID_ROLES: Role[] = [
  ...
  "TEAM_MEMBER", "DESIGNER", "MARKETING", "SALES_REP",
];

const userSelect = {
  id: true, name: true, email: true, image: true, role: true,
  lifecycleStatus: true, color: true, active: true, createdAt: true,
  customRoleId: true,
...
export async function PATCH(req: NextRequest) {
  ...
  if (role !== undefined) {
    ...
    if (role !== "CLIENT") {
      (updateData as Record<string, unknown>).lifecycleStatus = null;
    }
  }
...
export async function DELETE(req: NextRequest) {
  ...
  const target = await db.user.findFirst({ where: { id: userId }, select: { role: true, name: true, email: true } });
