...
function AppointmentEditModal({ open, onOpenChange, appointment, onSaved, onDeleted, initialDate }: AppointmentEditModalProps) {
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="bg-[var(--wl-surface)] border-[var(--wl-border)] text-[var(--wl-text-primary)] max-w-md max-h-[90vh] overflow-y-auto">
        <DialogHeader>
            ...
            <div className="grid grid-cols-2 gap-2">
              <div className="relative">
                <button type="button" onClick={() => setCalOpen(v=>!v)}
                  className="h-9 w-full flex items-center gap-2 rounded-md border border-[var(--wl-border)] bg-[var(--wl-hover)] px-3 text-[13px] text-left outline-none hover:border-brand/40"
              ...
              <div className="flex gap-1 h-9">
                ...
                <span className="flex items-center text-[var(--wl-text-placeholder)] text-xs">:</span>
                    ...
                    <button key={u.id} type="button" onClick={() => toggleUser(u.id)}
                      className={'flex items-center gap-1.5 px-2 py-1 rounded-full text-xs font-medium border transition-colors ' +
                        (selected
                          ...
                          : 'border-[var(--wl-border)] bg-[var(--wl-hover)] text-[var(--wl-text-muted)] hover:text-white hover:border-white/20')}>
