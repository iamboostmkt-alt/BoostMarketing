...
import { Hash, LifeBuoy, Users as UsersIcon, Briefcase, Lock, ChevronDown, ChevronRight, Building2, Menu, X } from 'lucide-react';
...
const BASE_CHANNELS: Channel[] = [
  ...
  { id: 'SUPPORT', type: 'SUPPORT', label: 'soporte',   hint: 'Atencion a clientes',      icon: LifeBuoy,  iconBg: 'bg-amber-400/15',  iconText: 'text-amber-300' },
  { id: 'PROJECT', type: 'PROJECT', label: 'proyectos', hint: 'Discusion de proyectos',   icon: Briefcase, iconBg: 'bg-purple-400/15', iconText: 'text-purple-300' },
...
export default function ChatWithChannels() {
  ...
  const activeTitle    = activeBase?.label
    ? `# ${activeBase.label}`
    : activeClient ? `# ${activeClient.name}` : '# chat';
    ...
    <>
      <div className="px-4 py-4 border-b border-white/[0.06] flex items-center justify-between">
        ...
        <button
          ...
          className="md:hidden p-1 rounded text-white/30 hover:text-white"
        >
          <X className="w-4 h-4" />
        </button>
      </div>
      ...
      <div className="flex-1 overflow-y-auto custom-scrollbar py-2">
            ...
            return (
              <button key={ch.id} onClick={() => selectChannel(ch.id)}
                className={`w-full flex items-center gap-2.5 px-2.5 py-2 rounded-lg transition-colors text-left ${
                  isActive ? 'bg-brand/20 text-white' : 'text-white/55 hover:bg-white/[0.04] hover:text-white/80'
                }`}>
                ...
                <div className="flex-1 min-w-0">
                  ...
                  {ch.hint && <p className="text-[10px] text-white/35 truncate">{ch.hint}</p>}
                </div>
              </button>
            );
          })}
        ...
        {clients.length > 0 && (
          <div className="mt-4">
            <button onClick={() => setShowClients((p) => !p)}
  ...
  return (
    <div className="flex h-[calc(100dvh-7rem)] md:h-[calc(100vh-7rem)] md:max-h-[820px] bg-[#0e0e14] rounded-2xl border border-white/[0.06] overflow-hidden relative">
      ...
      <aside className="hidden md:flex w-60 shrink-0 border-r border-white/[0.06] bg-[#0a0a10] flex-col">
        <SidebarContent />
      </aside>

      {/* Sidebar movil - drawer desde izquierda */}
      <aside className={`
        md:hidden fixed left-0 top-0 h-full w-64 z-50
        bg-[#0a0a10] border-r border-white/[0.06] flex flex-col
      ...
      <main className="flex-1 flex flex-col min-w-0 overflow-hidden">
        {/* Boton canales en movil */}
        <div className="md:hidden flex items-center gap-2 px-3 py-2 border-b border-white/[0.06] shrink-0">
          <button
            type="button"
            onClick={() => setSidebarOpen(true)}
            className="flex items-center gap-2 px-2.5 py-1.5 rounded-lg bg-white/[0.04] hover:bg-white/[0.08] transition-colors text-white/60 hover:text-white"
          >
            <Menu className="w-4 h-4" />
            <span className="text-xs font-medium">{activeTitle}</span>
          </button>
        </div>

        <div className="flex-1 px-3 md:px-5 py-3 md:py-4 overflow-hidden">
          <ChatContent
            key={activeId}
            room={activeId}
            title={activeTitle}
