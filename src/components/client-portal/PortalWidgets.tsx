  ...
  return (
    <div className="border border-[var(--wl-border)] rounded-xl overflow-hidden mt-2">
      <button type="button" onClick={() => setOpen(v => !v)}
        className="w-full flex items-center justify-between px-4 py-2.5 bg-[var(--wl-hover)] hover:bg-[var(--wl-hover)] transition-colors">
        <div className="flex items-center gap-2 text-xs font-medium text-[var(--wl-text-muted)]">
      ...
      {open && (
        <div className="space-y-1.5 p-3 border-t border-[var(--wl-border-subtle)]">
          {tasks.map((task: any) => (
            <div key={task.id} className="flex items-center gap-2.5 px-3 py-2 rounded-lg bg-[var(--wl-hover)] opacity-60">
              <CheckCircle2 className="w-3.5 h-3.5 text-green-400 shrink-0" />
              <span className="text-xs text-[var(--wl-text-muted)] line-through truncate flex-1">{task.title}</span>
              {task.dueDate && (
...
export function ProgressBar({ total, completed }: { total: number; completed: number }) {
  ...
  return (
    <div className="space-y-1.5">
      <div className="flex items-center justify-between text-xs text-[var(--wl-text-muted)]">
      ...
      <p className="text-[11px] text-[var(--wl-text-placeholder)]">{completed} de {total} ítems completados</p>
    </div>
  );
}
