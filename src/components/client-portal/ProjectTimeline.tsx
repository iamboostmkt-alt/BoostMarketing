    ...
    <div className="glass-card rounded-2xl p-5 space-y-4">
      <div className="flex items-center justify-between gap-2">
        <div className="flex items-center gap-2">
          ...
          <p className="text-xs font-medium text-[var(--wl-text-placeholder)] uppercase tracking-widest">Timeline</p>
        </div>
      ...
      <div className="relative">
        <div className="absolute left-3.5 top-2 bottom-2 w-px bg-[var(--wl-hover)]" />
              ...
              <div key={event.id} className="flex items-start gap-4 pl-1">
                <div className="relative z-10 mt-1.5 shrink-0">
                  {isCompleted ? (
                    ...
                    <div className="w-5 h-5 rounded-full bg-[var(--wl-hover)] border border-[var(--wl-border)] flex items-center justify-center">
                ...
                <div className={`flex-1 ${i === events.length - 1 ? 'pb-0' : 'pb-4'}`}>
                  <div className="flex items-start justify-between gap-2">
                    <p className={`text-sm leading-tight ${isCompleted ? 'text-[var(--wl-text-muted)] line-through' : isNext ? 'text-[var(--wl-text-primary)] font-medium' : 'text-[var(--wl-text-secondary)]'}`}>
                      {event.title}
                    </p>
                    {isNext && <span className="text-[10px] bg-brand/20 text-brand-light px-2 py-0.5 rounded-full shrink-0">Siguiente</span>}
                  </div>
                  <p className={`text-[11px] mt-0.5 ${isCompleted ? 'text-[var(--wl-text-placeholder)]' : 'text-[var(--wl-text-muted)]'}`}>
                  ...
                  {isManager && event.type === 'milestone' && (
                    <div className="flex gap-1.5 mt-1.5">
                      <button type="button" onClick={() => onEditMilestone?.(event.id)}
                        className="flex items-center gap-1 px-2 py-0.5 rounded text-[10px] bg-[var(--wl-hover)] hover:bg-[var(--wl-border)] border border-[var(--wl-border)] text-[var(--wl-text-muted)] hover:text-[var(--wl-text-primary)] transition-colors">
