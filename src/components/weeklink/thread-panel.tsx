...
function PanelLabel({ children }: { children: React.ReactNode }) {
  return <p className="mb-2 text-[11px] font-medium uppercase tracking-[0.08em] text-text-muted">{children}</p>
}
...
export function ThreadPanel() {
  return (
    <aside className="flex h-full w-[360px] shrink-0 flex-col border-l border-divider bg-[#11131a]">
      <header className="flex h-[60px] shrink-0 items-center justify-between border-b border-divider px-5">
        <h2 className="text-[15px] font-semibold tracking-tight">Hilo</h2>
        <button aria-label="Cerrar hilo" className="flex h-8 w-8 items-center justify-center rounded-lg text-text-secondary transition-colors hover:bg-white/[0.04] hover:text-foreground">
      ...
      <div className="flex-1 overflow-y-auto scrollbar-thin px-5 py-3">
        ...
        <div className="my-3 flex items-center gap-3">
          <span className="text-[11px] font-medium text-text-muted">6 respuestas</span>
        ...
        <div className="mt-5 border-t border-divider pt-4">
          ...
          <div className="mb-4 flex items-center">
            ...
            <span className="-ml-2 flex h-[26px] items-center justify-center rounded-full border border-border bg-surface-elevated px-2 text-[11px] font-medium text-text-secondary ring-2 ring-[#11131a]">
              +8
            </span>
          </div>

          <PanelLabel>Tarea relacionada</PanelLabel>
          <div className="mb-4 flex items-center gap-3 rounded-2xl border border-border bg-surface-elevated p-3">
            ...
            <div className="min-w-0 flex-1">
              <div className="flex items-center gap-2">
                ...
                <span className="rounded-md bg-primary/15 px-1.5 py-0.5 text-[10px] font-medium text-primary">En progreso</span>
              </div>
            ...
            {files.map((f) => (
              <li key={f.name}>
                <button className="flex w-full items-center gap-2.5 rounded-xl px-2 py-1.5 text-left transition-colors hover:bg-white/[0.03]">
                  ...
                  <span className="min-w-0 flex-1 truncate text-[12.5px] text-text-secondary">{f.name}</span>
                  <span className="text-[11px] text-text-muted">{f.size}</span>
                </button>
              </li>
            ))}
          </ul>
          ...
          <div className="flex flex-wrap gap-1.5">
            {['campaña', 'gymnastwin', 'ver2'].map((t) => (
              <span key={t} className="rounded-md border border-border bg-white/[0.03] px-2 py-1 text-[11px] text-text-secondary">
                {t}
              </span>
            ))}
          </div>
