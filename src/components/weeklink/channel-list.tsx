...
function GroupLabel({ children, action }: { children: React.ReactNode; action?: boolean }) {
  return (
    <div className="flex items-center justify-between px-2 pb-1 pt-4">
      <span className="text-[11px] font-medium uppercase tracking-[0.08em] text-text-muted">{children}</span>
      {action && (
        <button aria-label="Añadir" className="text-text-muted transition-colors hover:text-foreground">
...
export function ChannelList() {
  return (
    <div className="flex h-full w-[244px] shrink-0 flex-col border-r border-divider bg-card">
      <div className="flex-1 overflow-y-auto scrollbar-thin px-2 pb-4">
              ...
              <button
                className={cn(
                  ...
                  c.active
                    ...
                    : 'text-text-secondary hover:bg-white/[0.03] hover:text-foreground',
                )}
              >
                <Hash className={cn('h-4 w-4', c.active ? 'text-primary' : 'text-text-muted')} strokeWidth={1.75} />
        ...
        <ul className="flex flex-col gap-0.5">
          {clientChannels.map((c) => (
            <li key={c.id}>
              <button className="flex h-9 w-full items-center gap-2 rounded-[10px] px-2 text-[13px] text-text-secondary transition-colors hover:bg-white/[0.03] hover:text-foreground">
        ...
        <ul className="flex flex-col gap-0.5">
          {apps.map((a) => {
            ...
            return (
              <li key={a.id}>
                <button className="flex h-9 w-full items-center gap-2.5 rounded-[10px] px-2.5 text-[13px] text-text-secondary transition-colors hover:bg-white/[0.03] hover:text-foreground">
                  <Icon className={cn('h-4 w-4', a.id === 'ai' ? 'text-primary' : 'text-text-muted')} strokeWidth={1.75} />
