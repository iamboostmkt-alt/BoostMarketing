...
export function Composer({ placeholder, compact }: { placeholder: string; compact?: boolean }) {
  return (
    <div className="px-4 pb-4 pt-1">
      <div className="rounded-[18px] border border-input bg-white/[0.03] px-2 py-2 transition-colors focus-within:border-primary/40">
        <div className="flex items-center gap-1">
          {tools.slice(0, compact ? 4 : 5).map((t, i) => (
            <button
              key={i}
              aria-label={t.label}
              className="flex h-8 w-8 items-center justify-center rounded-lg text-text-secondary transition-colors hover:bg-white/[0.06] hover:text-foreground"
            >
          ...
          <input
            ...
            className="min-w-0 flex-1 bg-transparent px-2 text-[13.5px] text-foreground placeholder:text-text-muted focus:outline-none"
          />
          {!compact && (
            <>
              <button
